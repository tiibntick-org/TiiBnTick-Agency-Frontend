...
import { X, Package, MapPin, User, Truck, Clock, CheckCircle2, AlertTriangle, XCircle, Loader, Archive, Star } from 'lucide-react'
...
function getStepIndex(status: MissionStatus) {
...
export default function MissionDetailDrawer({ mission, open, onClose }: Props) {
  ...
  return (
    <>
      ...
      <div
        className={clsx(
          'fixed top-0 right-0 h-full w-[520px] bg-white z-50 flex flex-col shadow-xl border-l border-gray-200 transition-transform duration-300 ease-out',
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-200 flex-shrink-0">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-semibold text-gray-900 font-mono">{mission.manifestNumber}</h2>
              <span className={`text-xs font-medium px-2 py-0.5 rounded ${priority.cls}`}>{priority.label}</span>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">{mission.branchName} · {mission.id}</p>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-md transition-colors mt-0.5">
        ...
        <div className="border-t border-gray-200 px-6 py-4 bg-white flex-shrink-0">
          ...
          {mission.status === 'AT_HUB' && (
            <div className="flex gap-3">
              <button className="flex-1 py-2 text-sm font-medium text-white bg-violet-500 hover:bg-violet-600 rounded-lg transition-colors">
